...
VTN_plot <- vtn_df %>%
  ...
  labs( fill = "R$/ha")+
  theme_map()+
  theme(text=element_text(size=7),legend.position = c(0.1,0.10), legend.box = "horizontal")+
  # box da legenda transparente(nao funciona)
  theme(legend.key = element_rect(fill = "transparent"))

# x antes era 0.02, y 0.65

ggsave(plot = VTN_plot,filename = "/dados/pessoal/francisco/custo_oportunidade_terra/figures/report/var_resposta_MA_RF2019_2023.png",width = 10,height = 10,units = "cm", dpi = 250, bg = "white")
