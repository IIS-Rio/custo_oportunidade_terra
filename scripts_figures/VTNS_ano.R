# plotar os anos disponiveis e a complementaridade das informacoes de VTN
# correcao monetaria dos anos
# pacotes ----------------------------------------------------------------------
...
# plotando as figuras pros anos diferentes

# 2023

VTN_2023_df <- as.data.frame(VTN_2023, xy = TRUE)%>%
  filter(!is.na(.[[3]]))

small_constant <- 0.0001

Br <- read_country()
Br_pj <- st_transform(Br,crs(result_raster))


VTN_2023_p <- VTN_2023_df %>%
  mutate(value=.[[3]]+small_constant) %>%
  ggplot() +
  geom_sf(data=Br_pj, fill="lightgray",color=NA)+
  geom_tile(aes(x = x, y = y, fill = value)) +
  scale_fill_viridis_c(option = "magma", direction = -1,
                       labels = comma,trans = "log10") +
  labs(title = paste("Valor terra nua (2023): Brasil"), fill = "R$/ha")+
  theme_map()+
  theme(text=element_text(size=7))

# demorando mto pra salvar
ggsave(VTN_2023_p,"figures/VTN_2023_cobertura_disponivel.tif",width = "16",height = "9",units = "cm", dpi = 150, device = "png",type = "cairo")

# 2022

VTN_2022_df <- as.data.frame(VTN_2022, xy = TRUE)%>%
  filter(!is.na(.[[3]]))

VTN_2022_p <- VTN_2022_df %>%
  mutate(value=.[[3]]+small_constant) %>%
  ggplot() +
  geom_sf(data=Br_pj, fill="lightgray",color=NA)+
  geom_tile(aes(x = x, y = y, fill = value)) +
...
# complemento 2019_2021

VTN_2019_2021_df <- as.data.frame(VTN_2019_2021, xy = TRUE)%>%
  filter(!is.na(.[[3]]))

VTN_2019_2021_df_p <- VTN_2019_2021_df %>%
  mutate(value=.[[3]]+small_constant) %>%
  ggplot() +
  geom_sf(data=Br_pj, fill="lightgray",color=NA)+
  geom_tile(aes(x = x, y = y, fill = value)) +
  scale_fill_viridis_c(option = "magma", direction = -1,
                       labels = comma,trans = "log10") +
  labs(title = paste("Valor terra nua (2019,2021): Brasil"), fill = "R$/ha")+
  theme_map()+
  theme(text=element_text(size=7))

# juncao

result_raster_df <- as.data.frame(result_raster, xy = TRUE)%>%
  filter(!is.na(.[[3]]))

VTN_2019_2023_p <-result_raster_df %>%
  mutate(value=.[[3]]+small_constant) %>%
  ggplot() +
  geom_sf(data=Br_pj, fill="lightgray",color=NA)+
  geom_tile(aes(x = x, y = y, fill = value)) +
  scale_fill_viridis_c(option = "magma", direction = -1,
                       labels = comma,trans = "log10") +
  labs(title = paste("Valor terra nua (2019,2021,2022,2023): Brasil"), fill = "R$/ha")+
  theme_map()+
  theme(text=element_text(size=7))
