...
ma <- raster("/dados/projetos_andamento/custo_oportunidade/resultados_regreen/predicted_VTN_MA.tif")
...
my_palette <- scale_fill_gradientn(
  ...
  name = "R$/ha",
  breaks = c(150, 1500, 15000,150000),  # Specify the desired break points
  labels = comma,
  trans = "log10"
)
...
VTN_plot <- vtn_df %>%
  # removendo NAs
  filter(!is.na(predicted_VTN_MA))%>%
  #filter(regiao==rg) %>%
  mutate(value=predicted_VTN_MA+small_constant) %>%
  ggplot() +
