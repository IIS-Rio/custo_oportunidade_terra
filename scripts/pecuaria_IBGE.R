...
cer_mun <- st_read(cer_mun,"/dados/projetos_andamento/TRADEhub/GLOBIOMbr/oc/mun_cerrado.shp")
...
num_groups = round(length(lista_mun)/500,0)
...
category  <-  list(2682,2684) # leite, la
...
category  <-  list(2684) #  la

# rodando 

# aqui nao baixou la
la <- lapply(mun_code,f,tabela=tabela_leite,classificacao=classific,categoria=category)
...
valor_area_pastagem <- left_join(valor_producao,area_pastagem_df[,c(1,2,5,6,7)])
...
valor_area_pastagem$reais_ha[is.na(valor_area_pastagem$reais_ha)] <- 0
