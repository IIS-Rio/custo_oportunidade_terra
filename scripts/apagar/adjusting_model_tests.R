################################## ----
# testes que surgiram do ws ----

# 1) treinar e testar para regiões IBGE
# 2) avaliar/testar a amostragem de pontos feita por municipio (se segue com o mesmo numero de pontos, se faz estratificada, etc.)
# 3) determinar qual análise estatistica dos resultados faz mais sentido para esse tipo de dado -- se a avaliação será feita olhando desvio padrão, resíduos etc. 
# 4) avaliar qualitativamente as variáveis preditoras utilizadas e se faltam algumas como desmatamento, fogo, etc.. -- Importante que a avaliação qualitativa seja feita considerando a divisão por regiões proposta.
# 5) pensar na validação dos resultados
# 6) avaliar se há alguma questão relacionada ao randomforest na determinação de modelos econométricos -- como lidar com correlação espacial etc.
# 7) há como considerar especulação imobiliária no treinamento do modelo
# 8) após novos testes, avaliar se o Rio de Janeiro continua com resultados divergentes do esperado
