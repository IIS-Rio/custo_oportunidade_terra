...
reg_nm <- c("CO","NE","N","SE","SUL")

# raster base pra rasterizar
...
for(i in seq_along(f))  {
  ...
  reg = fread(f[i]) # mudar dependendo da regiao! (da pra inserir num loop)
  
  reg <- left_join(reg,vr)
  
  names(reg)[grep(pattern = "energia",x = names(reg))]<- "prop_com_energia"
  names(reg)[grep(pattern = "superior",x = names(reg))] <- "prop_com_ens_superior"
  ...
  n_bootstraps <- 1 # por enquanto nenhum!
  
  bootstrap_predictions <- matrix(nrow = nrow(reg_imput_sc), ncol = n_bootstraps)
  ...
  for(j in 1:n_bootstraps){
    ...
    bootstrap_predictions[,j] <- predicted_reg$predicted
...
#   Add the confidence limits as new columns to the matrix
  ...
  reg_pred <- cbind(reg,"Median_VTN"=bootstrap_predictions[,1])
  ...
  VTN<-rasterize(predicted_df_sp, r, field = "Median_VTN_exp")
  ...
  writeRaster(x = VTN,filename = paste0("/dados/projetos_andamento/custo_oportunidade/rasters_VTN/regioes_5/predicted_VTN_multi_ano_v2_",reg_nm[[i]],".tif"),overwrite=T)
  ...
  rm(reg,reg_imput,reg_imput_sc,reg_imput_sc,reg_pred,reg_sc_continuous,rfModel_full,predicted_df_sp,predicted_reg,predicted_reg_IC,VTN,bootstrap_data,bootstrap_predictions)
